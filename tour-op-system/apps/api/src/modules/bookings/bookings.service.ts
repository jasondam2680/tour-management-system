...
export class BookingsService {
  ...
  async findOne(id: string, organizationId: string) {
    const booking = await this.prisma.booking.findFirst({
      ...
      include: {
        tour:     { select: { id: true, code: true, title: true, status: true, travelDateFrom: true, travelDateTo: true } },
        supplier: true,
  ...
  async update(id: string, organizationId: string, dto: UpdateBookingDto) {
    ...
    if (booking.status === BookingStatus.CANCELLED) throw new BadRequestException('Không thể sửa booking đã huỷ');
    ...
    return this.prisma.booking.update({
      where: { id },
      data: {
        ...restDto, quantity, unitCost, totalCost,
  ...
  async addPayment(id: string, organizationId: string, data: {
    amount: number; currency: string; method: string; reference?: string; notes?: string; dueDate?: string;
  }) {
    const booking = await this.findOne(id, organizationId);
  ...
  async getStats(organizationId: string) {
    const [total, byStatus, unpaidAmount] = await Promise.all([
      ...
      this.prisma.booking.groupBy({ by: ['status'], where: { tour: { organizationId } }, _count: true }),

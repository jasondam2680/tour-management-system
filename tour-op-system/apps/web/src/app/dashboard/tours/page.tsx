...
function formatMoney(amount: number | string, currency: string) {
  return (
    new Intl.NumberFormat('vi-VN', { style: 'decimal', maximumFractionDigits: 0 }).format(
      Number(amount),
    ) +
    ' ' +
    currency
...
export default function ToursPage() {
  ...
  const load = useCallback(async (q: QueryToursParams) => {
    setLoading(true);
    try {
      const [result, statsResult] = await Promise.all([toursApi.getAll(q), toursApi.getStats()]);
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <StatCard
            ...
            sub="tours đã hoàn thành"
          />
          <StatCard
            label="Lợi nhuận"
            value={formatMoney(stats.totalProfit, 'USD')}
              ...
              {tours.map((tour) => (
                <tr key={tour.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-700 font-medium">
                    {formatMoney(tour.sellingPrice, tour.currency)}
                  </td>
                  <td className="px-4 py-3">
                    <span

...
const NEXT_STATUSES: Partial<
  Record<TourStatus, { status: TourStatus; label: string; color: string }[]>
> = {
  PLANNING: [
    { status: 'CONFIRMED', label: 'Xác nhận tour', color: 'bg-blue-600 hover:bg-blue-700' },
  ],
  CONFIRMED: [
    { status: 'IN_PROGRESS', label: 'Bắt đầu tour', color: 'bg-green-600 hover:bg-green-700' },
  ],
  IN_PROGRESS: [
...
export default function TourDetailPage() {
  ...
  async function loadTour() {
    setLoading(true);
    try {
      setTour(await toursApi.getOne(id));
    } catch {
  ...
  async function handleStatusChange(status: TourStatus) {
    setChanging(true);
    try {
      await toursApi.changeStatus(tour.id, status);
      await loadTour();
    } finally {
      setChanging(false);
    }
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <span
              className={`px-2 py-1 rounded-full text-xs font-medium ${STATUS_COLOR[tour.status as TourStatus]}`}
            >
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={!cancelReason.trim() || changing}
          ...
          {tour.quotation && (
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-sm font-medium text-gray-900 mt-1">
                {formatMoney(tour.quotation.totalAmount, tour.quotation.currency)}
              </p>
            </div>
          )}

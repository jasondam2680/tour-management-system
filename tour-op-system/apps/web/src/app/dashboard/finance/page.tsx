...
function formatMoney(v: any, c = 'USD') {
  return new Intl.NumberFormat('vi-VN').format(Math.round(toNum(v))) + ' ' + c;
}
    ...
    <div className="space-y-2">
      ...
      <div className="flex gap-4 text-xs text-gray-500">
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 bg-blue-200 rounded-sm inline-block" /> Hoá đơn
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 bg-blue-500 rounded-sm inline-block" /> Đã thu
        </span>
      </div>
    </div>
  );
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">AR (Receivables) · AP (Payables) · Overview</p>
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <MetricCard
            ...
            accent="bg-blue-50"
            value={formatMoney(ar.totalAmount)}
            sub={`${ar.countTotal} invoices`}
          />
          <MetricCard
            icon="✅"
            label="Paid"
            accent="bg-green-50"
            value={formatMoney(ar.totalPaid)}
            sub={`${ar.countPaid} invoices`}
          />
          <MetricCard
            icon="⏳"
            label="Due"
            accent="bg-yellow-50"
            value={formatMoney(ar.totalDue)}
            sub={`${ar.countUnpaid} invoices`}
          />
          <MetricCard
            icon="🔴"
            label="Overdue"
            accent="bg-red-50"
            value={ar.countOverdue}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <MetricCard
            icon="🏢"
            label="Total Supplier Costs"
            accent="bg-orange-50"
            value={formatMoney(ap.totalCost, 'VND')}
            sub={`${ap.countTotal} bookings`}
          />
          <MetricCard
            icon="✅"
            label="Paid"
            accent="bg-green-50"
            value={formatMoney(ap.totalPaid, 'VND')}
            sub={`${ap.countPaid} bookings`}
          />
          <MetricCard
            icon="⏳"
            label="Due"
            accent="bg-red-50"
            value={formatMoney(ap.totalDue, 'VND')}
            sub={`${ap.countUnpaid} unpaid`}
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-red-200 rounded-xl p-5">
          ...
          {!overview.overdueInvoices.length ? (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              {['Code / Customer', 'Tour', 'Total Amount', 'Remaining', 'Status', ''].map((h) => (
                <th key={h} className="text-left px-5 py-3 font-medium text-gray-500 text-xs">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-gray-100">
            {overview.recentInvoices.map((inv) => (
              <tr key={inv.id} className="hover:bg-gray-50">
                ...
                <td className="px-5 py-3">
                  <span
                    className={`text-xs px-2 py-1 rounded-full font-medium ${PAYMENT_BADGE[inv.status] ?? ''}`}
                  >
                    {PAYMENT_LABEL[inv.status] ?? inv.status}
                  </span>
                </td>
                <td className="px-5 py-3">
                  <Link

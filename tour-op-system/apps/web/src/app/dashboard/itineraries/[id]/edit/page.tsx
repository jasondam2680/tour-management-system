...
export default function EditItineraryPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = (await itinerariesApi.getOne(id)) as any;
        setItinerary(data);
        setTitle(data.title || '');
        setOverview(data.currentVersion?.overview || '');
        setNotes(data.currentVersion?.notes || '');

        if (data.currentVersion?.days) {
          setDays(
            data.currentVersion.days.map((d: any) => ({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      // Create new version with updated days
      await itinerariesApi.createVersion(id, {
        title: title.trim(),
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          {itinerary?.code} &middot; Tạo version mới với thay đổi
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex gap-3">
          ...
          >
            {loading ? 'Đang tạo version...' : 'Tạo Version mới'}
          </button>
        </div>
      </form>
    </div>

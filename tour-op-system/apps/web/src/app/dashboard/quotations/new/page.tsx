...
function transformItem(item: ItemData, pax: number) {
  ...
  if (cat === 'hotel' || cat === 'resort') {
    ...
    return { ...base, quantity: rooms * nights, unit: 'per_room',
      description: checkIn ? `Check-in: ${checkIn} | Check-out: ${checkOut} | ${rooms} phòng × ${nights} đêm` : item.description };
  }
  if (cat === 'guide') {
    const days = numDays ?? toNum(item.quantity);
    return { ...base, quantity: days, unit: 'per_day',
      notes: (tipsPerPerson ?? 0) > 0 ? `Tips: ${tipsPerPerson} sell / ${buyTipsPerPerson} buy × ${pax} khách` : item.notes };
  }
...
function CustomerSearch({ value, onChange, error }: { value: string; onChange: (id: string, c: CustomerResult | null) => void; error?: string; }) {
      ...
      {selected ? (
        <div className="flex items-center justify-between px-3 py-2.5 border border-blue-400 bg-blue-50 rounded-lg">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900">{displayName(selected)}{selected.isVip&&' ⭐'}</p>
              <p className="text-xs text-slate-400">{selected.code}{selected.email&&` · ${selected.email}`}</p>
            </div>
          </div>
          ...
          {results.map((c) => (
            <button key={c.id} type="button" onClick={() => handleSelect(c)}
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-900 truncate">{displayName(c)}{c.isVip&&' ⭐'}</p>
...
function SmartItemRow({ idx, register, watch, setValue, remove, pax, quotationCurrency }: {
  idx: number; register: any; watch: any; setValue: any; remove: () => void; pax: number; quotationCurrency: string;
}) {
  ...
  useEffect(() => {
    if (!cat) return;
    api.get<any>('/suppliers', { category: cat.toUpperCase(), limit: 100, isActive: true })
      .then((res: any) => setSuppliers(res?.data ?? [])).catch(() => {});
  }, [cat]);

  const supplierId = item?.supplierId;
  useEffect(() => {
    if (!supplierId) { setResources([]); return; }
    api.get<any>(`/suppliers/${supplierId}`)
      .then((res: any) => setResources(res?.resources ?? [])).catch(() => {});
  }, [supplierId]);
  ...
  const rowTotal = (() => {
    if (cat === 'hotel' || cat === 'resort')
      return toNum(item?.sellingPrice) * (item?.numRooms ?? 1) * getNights(item?.checkIn, item?.checkOut);
    return toNum(item?.sellingPrice) * toNum(item?.quantity);
  })();

  const margin = toNum(item?.buyingPrice) > 0
    ? (((toNum(item?.sellingPrice) - toNum(item?.buyingPrice)) / toNum(item?.buyingPrice)) * 100).toFixed(0) : '—';
    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-slate-50 border-b border-slate-200">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-slate-600 uppercase tracking-wide">{CAT_MAP[cat]?.label ?? cat}</span>
          <input {...register(`items.${idx}.day`)} type="number" min={1} placeholder="Ngày #"
        ...
        <div className="flex items-center gap-3">
          <span className={`text-xs px-2 py-0.5 rounded font-medium ${Number(margin)>=20?'bg-emerald-100 text-emerald-700':Number(margin)>=10?'bg-amber-100 text-amber-600':'bg-slate-100 text-slate-500'}`}>
            Markup: {margin}%
          </span>
      ...
      <div className="p-4 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select value={item?.supplierId??''} onChange={(e) => { setValue(`items.${idx}.supplierId`, e.target.value); setValue(`items.${idx}.resourceId`, ''); setValue(`items.${idx}.name`, ''); }} className={inputCls}>
        ...
        {resources.length > 0 && item?.resourceId && (
          <div><label className={labelCls}>Tên hiển thị</label><input {...register(`items.${idx}.name`)} className={inputCls} /></div>
        )}
        ...
        {(cat==='hotel'||cat==='resort') && (
          <div className="space-y-3 p-3 bg-blue-50 rounded-lg border border-blue-100">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div><label className={labelCls}>Số phòng</label><input type="number" min={1} {...register(`items.${idx}.numRooms`)} placeholder="1" className={inputCls} /></div>
            </div>
            {item?.checkIn && item?.checkOut && (
              <div className="text-xs text-blue-600 bg-blue-100 rounded px-3 py-1.5">
                {getNights(item.checkIn,item.checkOut)} đêm × {item?.numRooms??1} phòng = <strong>{getNights(item.checkIn,item.checkOut)*(item?.numRooms??1)}</strong> room-nights
                → <strong>{formatCurrency(toNum(item?.sellingPrice)*getNights(item.checkIn,item.checkOut)*(item?.numRooms??1),item?.currency||quotationCurrency)}</strong>
              </div>
            )}
            <div className="grid grid-cols-3 gap-3">
        ...
        {(cat==='transport'||cat==='boat') && (
          <div className="space-y-3 p-3 bg-orange-50 rounded-lg border border-orange-100">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div><label className={labelCls}>Đơn vị</label>
                <select {...register(`items.${idx}.unit`)} className={inputCls}>
                  <option value="per_trip">/ chuyến</option><option value="per_vehicle">/ xe</option><option value="per_day">/ ngày</option>
                </select>
              </div>
            </div>
        ...
        {cat==='restaurant' && (
          <div className="space-y-3 p-3 bg-green-50 rounded-lg border border-green-100">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div><label className={labelCls}>Đơn vị</label>
                <select {...register(`items.${idx}.unit`)} className={inputCls}>
                  <option value="per_person">/ người</option><option value="per_table">/ bàn</option><option value="per_trip">/ buổi</option>
                </select>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
        ...
        {cat==='guide' && (
          <div className="space-y-3 p-3 bg-purple-50 rounded-lg border border-purple-100">
            ...
            <div><label className={labelCls}>Tiền tệ</label><select {...register(`items.${idx}.currency`)} className={`${inputCls} w-40`}>{CURRENCIES.map((c)=><option key={c}>{c}</option>)}</select></div>
          </div>
        )}
        ...
        {(cat==='visa'||cat==='insurance'||cat==='other') && (
          <div className="space-y-3 p-3 bg-slate-50 rounded-lg border border-slate-200">
            ...
            <div><label className={labelCls}>Mô tả</label><input {...register(`items.${idx}.description`)} className={inputCls} /></div>
          </div>
        )}

        <div className="flex items-center gap-6">
          <label className="flex items-center gap-1.5 text-xs text-slate-600 cursor-pointer">
            <input type="checkbox" {...register(`items.${idx}.isIncluded`)} className="rounded" defaultChecked />
            Bao gồm trong giá
          </label>
          <label className="flex items-center gap-1.5 text-xs text-slate-600 cursor-pointer">
...
export default function NewQuotationPage() {
  ...
  const addItem = useCallback((category: string) => {
    append({ category, name:'', quantity:1, unit:'per_person', sellingPrice:0, buyingPrice:0,
      currency:watchedCurrency, isOptional:false, isIncluded:true, numRooms:1, numDays:1 });
    setActiveTab('items');
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <CustomerSearch value={watch('customerId')??''} onChange={(id) => setValue('customerId', id, {shouldValidate:true})} error={errors.customerId?.message} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1.5">Lead ID (tuỳ chọn)</label>
                ...
                <>
                  ...
                  <div className="space-y-4">
                    ...
                    {fields.map((field, idx) => (
                      <SmartItemRow key={field.id} idx={idx} register={register} watch={watch} setValue={setValue}
                        remove={() => remove(idx)} pax={watchedPax} quotationCurrency={watchedCurrency} />
                    ))}
                  </div>
                </>
              )}
          ...
          <div className="p-5">
            ...
            {watchedPax > 0 && totals.total > 0 && (
              <div className="mt-4 bg-blue-50 rounded-xl p-3.5">
                ...
                <p className="text-xl font-bold text-blue-700 mt-0.5">{formatCurrency(totals.total/watchedPax, watchedCurrency)}</p>
              </div>
            )}
            <div className="mt-6 pt-5 border-t border-dashed border-slate-200">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between font-semibold"><span>Lợi nhuận</span>
                  <span className={totals.profit>=0?'text-emerald-600':'text-red-600'}>{formatCurrency(totals.profit, watchedCurrency)}</span>
                </div>
              </div>
              <div className={`mt-4 rounded-xl p-4 text-center ${totals.margin>=20?'bg-emerald-50':totals.margin>=10?'bg-amber-50':'bg-red-50'}`}>
                ...
                <p className={`text-xs font-medium mt-1 ${totals.margin>=20?'text-emerald-600':totals.margin>=10?'text-amber-600':'text-red-600'}`}>

...
const STATUS_LABEL: Record<string, string> = {
  DRAFT:'Nháp', SENT:'Đã gửi', VIEWED:'Đã xem', NEGOTIATING:'Đang thương lượng',
  APPROVED:'Đã duyệt', REJECTED:'Từ chối', EXPIRED:'Hết hạn', CONVERTED:'Đã tạo Tour',
};
const STATUS_COLOR: Record<string, string> = {
  DRAFT:'bg-gray-100 text-gray-600', SENT:'bg-blue-100 text-blue-700',
  VIEWED:'bg-cyan-100 text-cyan-700', NEGOTIATING:'bg-yellow-100 text-yellow-700',
...
const NEXT_ACTIONS: Partial<Record<string, {status:string;label:string;color:string}[]>> = {
  ...
  SENT:        [{status:'APPROVED',  label:'✅ Duyệt',         color:'bg-green-600 hover:bg-green-700'},
  ...
  VIEWED:      [{status:'APPROVED',  label:'✅ Duyệt',         color:'bg-green-600 hover:bg-green-700'},
                {status:'NEGOTIATING',label:'💬 Thương lượng', color:'bg-yellow-500 hover:bg-yellow-600'},
                {status:'REJECTED',  label:'❌ Từ chối',       color:'bg-red-500 hover:bg-red-600'}],
  NEGOTIATING: [{status:'APPROVED',  label:'✅ Duyệt',         color:'bg-green-600 hover:bg-green-700'},
                {status:'REJECTED',  label:'❌ Từ chối',       color:'bg-red-500 hover:bg-red-600'}],
  APPROVED:    [{status:'CONVERTED', label:'🗺️ Tạo Tour',      color:'bg-purple-600 hover:bg-purple-700'}],
};
...
export default function QuotationDetailPage() {
  ...
  async function handleStatusChange(status: string, reason?: string) {
    setChanging(true);
    try { await api.patch(`/quotations/${id}/status`, { status, reason }); await load(); }
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <button onClick={() => router.push('/dashboard/quotations')} className="text-sm text-gray-400 hover:text-gray-600 mb-2 block">← Danh sách báo giá</button>
          ...
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            ...
            <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_COLOR[quotation.status] ?? 'bg-gray-100 text-gray-600'}`}>
              {STATUS_LABEL[quotation.status] ?? quotation.status}
            </span>
            {quotation.destination && <span className="text-sm text-gray-500">📍 {quotation.destination}</span>}
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => { setShowReject(false); setRejectReason(''); }}
                className="flex-1 border border-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm">Huỷ</button>
              <button disabled={!rejectReason.trim() || changing}
                onClick={async () => { await handleStatusChange('REJECTED', rejectReason); setShowReject(false); setRejectReason(''); }}
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div className="px-5 py-3.5 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
              <h3 className="font-semibold text-gray-800 text-sm">Dịch vụ ({items.length} items)</h3>
              <span className="text-xs text-gray-400">
                {quotation.pax} khách · {formatDate(quotation.travelDateFrom)}{quotation.travelDateTo ? ` → ${formatDate(quotation.travelDateTo)}` : ''}
              </span>
            </div>
            {items.length === 0 ? (
                  ...
                  <div key={dayKey}>
                    {dayKeys.length > 1 && (
                      <div className="px-5 py-2 bg-slate-50 border-b border-slate-100">
                        <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">📅 {dayKey}</span>
                      </div>
                    )}
                    {itemsByDay[dayKey].map((item: any) => {
                      ...
                      const margin = toNum(item.buyingPrice) > 0
                        ? ((toNum(item.sellingPrice) - toNum(item.buyingPrice)) / toNum(item.buyingPrice) * 100)
                        : 0;
                      return (
                        <div key={item.id} className={`px-5 py-4 flex items-start gap-4 ${!item.isIncluded ? 'opacity-50' : ''}`}>
                          <span className="text-xl flex-shrink-0 mt-0.5">{CAT_ICON[item.category?.toLowerCase()] ?? '📦'}</span>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-2">
                              <div>
                                <p className="text-sm font-medium text-gray-900">
                                  {item.name}
                                  {item.isOptional && <span className="ml-1.5 text-xs text-gray-400 font-normal">(tuỳ chọn)</span>}
                                  {!item.isIncluded && <span className="ml-1.5 text-xs text-orange-500 font-normal">(không tính giá)</span>}
                                </p>
                                {item.description && <p className="text-xs text-gray-400 mt-0.5">{item.description}</p>}
                                <p className="text-xs text-gray-400 mt-0.5">
                                  {toNum(item.quantity)} × {formatMoney(item.sellingPrice, item.currency)}
                                  <span className={`ml-1 font-medium ${margin >= 20 ? 'text-emerald-600' : margin >= 10 ? 'text-amber-600' : 'text-red-500'}`}>
                                    ({margin.toFixed(0)}% markup)
                                  </span>
                                </p>
                              </div>
                              <div className="text-right flex-shrink-0">
                                <p className="text-sm font-semibold text-gray-900">{formatMoney(total, item.currency)}</p>
                                <p className="text-xs text-gray-400">vốn: {formatMoney(toNum(item.buyingPrice) * toNum(item.quantity), item.currency)}</p>
                              </div>
                            </div>
                          </div>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {quotation.internalNotes && (
                <div className="bg-white border border-gray-200 rounded-xl p-5">
                  ...
                  <p className="text-sm text-gray-600 whitespace-pre-line">{quotation.internalNotes}</p>
                </div>
              )}
            </div>
          )}
        ...
        <div className="space-y-5">
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            ...
            <div className="p-5 space-y-0">
              <InfoRow label="Subtotal" value={formatMoney(quotation.subtotal, quotation.currency)} />
              {toNum(quotation.discountAmount) > 0 && (
                <InfoRow label={`Giảm giá (${toNum(quotation.discountPct)}%)`} value={<span className="text-red-500">−{formatMoney(quotation.discountAmount, quotation.currency)}</span>} />
              )}
              {toNum(quotation.taxAmount) > 0 && (
                <InfoRow label={`Thuế (${toNum(quotation.taxPct)}%)`} value={formatMoney(quotation.taxAmount, quotation.currency)} />
              )}
              <div className="flex justify-between pt-3 mt-1 border-t border-gray-200">
                <span className="font-bold text-gray-900">Tổng cộng</span>
                <span className="text-lg font-bold text-blue-600">{formatMoney(quotation.totalAmount, quotation.currency)}</span>
              </div>
              {toNum(quotation.pax) > 0 && (
                <div className="mt-3 bg-blue-50 rounded-lg px-4 py-3">
                  <p className="text-xs text-blue-500">Giá / người</p>
                  <p className="text-lg font-bold text-blue-700">{formatMoney(toNum(quotation.totalAmount) / toNum(quotation.pax), quotation.currency)}</p>
                </div>
              )}
            </div>
            <div className="px-5 pb-5 border-t border-dashed border-gray-200">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3 pt-4">🔒 Nội bộ</p>
              <InfoRow label="Tổng vốn" value={formatMoney(quotation.totalCost, quotation.currency)} />
              <InfoRow label="Lợi nhuận" value={<span className={toNum(quotation.profitAmount) >= 0 ? 'text-emerald-600 font-semibold' : 'text-red-600 font-semibold'}>{formatMoney(quotation.profitAmount, quotation.currency)}</span>} />
              <div className={`mt-3 rounded-xl p-4 text-center ${toNum(quotation.profitMargin) >= 20 ? 'bg-emerald-50' : toNum(quotation.profitMargin) >= 10 ? 'bg-amber-50' : 'bg-red-50'}`}>
                <p className={`text-3xl font-black ${toNum(quotation.profitMargin) >= 20 ? 'text-emerald-600' : toNum(quotation.profitMargin) >= 10 ? 'text-amber-600' : 'text-red-600'}`}>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            <InfoRow label="Số khách" value={`${quotation.pax} (${quotation.paxAdult ?? 0} NL · ${quotation.paxChild ?? 0} TE)`} />

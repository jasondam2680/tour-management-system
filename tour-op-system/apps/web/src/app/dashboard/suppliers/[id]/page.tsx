...
import { suppliersApi, SupplierDetail, Resource, ResourceTemplate } from '@/lib/api/suppliers';
...
const UNIT_LABEL: Record<string,string> = {
  per_person:'/ người', per_room:'/ phòng', per_trip:'/ chuyến',
  per_day:'/ ngày', per_vehicle:'/ xe', per_table:'/ bàn', per_bed:'/ giường',
};

function formatMoney(v: number, c: string) { return new Intl.NumberFormat('vi-VN').format(v) + ' ' + c; }
...
function ResourceModal({supplierId,category,templates,onClose,onSaved,editing}:{
  supplierId:string; category:string; templates:ResourceTemplate[];
  onClose:()=>void; onSaved:()=>void; editing:Resource|null;
}) {
  const [form, setForm] = useState({
    name:editing?.name??'', description:editing?.description??'',
    basePrice:editing?.basePrice??0, currency:editing?.currency??'VND',
    unit:editing?.unit??'per_person', capacity:editing?.capacity??'',
    location:editing?.location??'', category:editing?.category??category,
  });
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg space-y-4 p-6 max-h-[90vh] overflow-y-auto">
        <h3 className="font-semibold text-gray-900 text-lg">{editing?'Cập nhật dịch vụ':'Thêm dịch vụ / sản phẩm'}</h3>

        {!editing && templates.length > 0 && (
          <div>
            <p className="text-xs text-gray-500 mb-2">Chọn nhanh từ gợi ý:</p>
            <div className="flex flex-wrap gap-1.5 max-h-28 overflow-y-auto">
              {templates.map((t) => (
                <button key={t.name} onClick={() => setForm((f) => ({...f, name:t.name, unit:t.unit}))}
                  className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
                    form.name===t.name ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300 hover:border-blue-400'
                  }`}>{t.name}</button>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Tên dịch vụ <span className="text-red-500">*</span></label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Giá cơ bản <span className="text-red-500">*</span></label>
            ...
            <div>
              ...
              <select value={form.currency} onChange={(e) => setForm((f) => ({...f, currency:e.target.value}))} className={inputCls}>
                {['VND','USD','EUR','THB','CNY'].map((c) => <option key={c}>{c}</option>)}
              </select>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="number" min={0} value={form.capacity} onChange={(e) => setForm((f) => ({...f, capacity:e.target.value}))} placeholder="VD: 45" className={inputCls} />
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Địa điểm</label>
            <input value={form.location} onChange={(e) => setForm((f) => ({...f, location:e.target.value}))} placeholder="VD: Tầng 5, Tòa nhà A" className={inputCls} />
          </div>
        </div>
...
export default function SupplierDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSupplier(data);
      const tmpl = await suppliersApi.getTemplates(data.category);
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <button onClick={() => router.push('/dashboard/suppliers')} className="text-sm text-gray-400 hover:text-gray-600 mb-2 block">← Danh sách NCC</button>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{supplier.category}</span>
                {supplier.isPreferred && <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-medium">⭐ Ưu tiên</span>}
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
        {([['resources',`🛎️ Dịch vụ (${activeResources.length})`],['info','ℹ️ Thông tin']] as [Tab,string][]).map(([t,l]) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab===t?'bg-white text-gray-900 shadow-sm':'text-gray-500 hover:text-gray-700'}`}>{l}</button>
        ))}
      </div>
      ...
      {tab==='resources' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-500">Sản phẩm / dịch vụ của <strong>{supplier.name}</strong></p>
            <button onClick={() => { setEditingResource(null); setShowModal(true); }}
              className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium">+ Thêm dịch vụ</button>
          </div>

          {activeResources.length === 0 ? (
            <div className="bg-white border-2 border-dashed border-gray-200 rounded-xl p-12 text-center">
              <p className="text-gray-400 text-sm">Chưa có dịch vụ nào</p>
              <button onClick={() => { setEditingResource(null); setShowModal(true); }} className="mt-3 text-blue-600 text-sm hover:underline">Thêm dịch vụ đầu tiên →</button>
            </div>
          ) : (
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <thead><tr className="bg-gray-50 border-b border-gray-200">
                  {['Tên dịch vụ','Đơn vị','Giá cơ bản','Sức chứa','Địa điểm',''].map((h) => (
                ...
                <tbody className="divide-y divide-gray-100">
                  {activeResources.map((r) => (
                    <tr key={r.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 font-medium text-gray-900">{formatMoney(r.basePrice, r.currency)}</td>
                      <td className="px-4 py-3 text-gray-600">{r.capacity??'—'}</td>
                      <td className="px-4 py-3 text-gray-500 text-xs">{r.location??'—'}</td>
                      <td className="px-4 py-3">
                        <div className="flex gap-2 justify-end">
                          <button onClick={() => { setEditingResource(r); setShowModal(true); }} className="text-xs text-blue-600 hover:underline">Sửa</button>
                          <button onClick={() => handleRemoveResource(r.id)} className="text-xs text-red-500 hover:underline">Xoá</button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {tab==='info' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            <InfoRow label="Đánh giá"  value={`${'★'.repeat(supplier.rating)}${'☆'.repeat(5-supplier.rating)}`} />
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-5">
      ...
      {showModal && supplier && (
        <ResourceModal
          supplierId={supplier.id} category={supplier.category}
          templates={templates} editing={editingResource}
          onClose={() => { setShowModal(false); setEditingResource(null); }}
          onSaved={() => { setShowModal(false); setEditingResource(null); load(); }}
        />
      )}
    </div>
  );

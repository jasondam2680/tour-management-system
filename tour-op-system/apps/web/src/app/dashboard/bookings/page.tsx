      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            {
              label: 'Công nợ NCC',
              value: formatMoney(stats.totalUnpaid, 'VND'),
              color: 'text-red-600',
            },
                ...
                <tr key={b.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3">
                    <div className="font-medium text-gray-900">
                      {CATEGORY_ICON[b.category]} {b.title}
                    </div>
                  ...
                  <td className="px-4 py-3">
                    {b.tour ? (
                      <Link
                        href={`/dashboard/tours/${b.tour.id}`}
                  ...
                  <td className="px-4 py-3 text-gray-700">
                    ...
                    {b.supplier?.phone && (
                      <div className="text-xs text-gray-400">{b.supplier.phone}</div>
                    )}
                  </td>
                  <td className="px-4 py-3 text-gray-700">
                  ...
                  <td className="px-4 py-3 font-medium text-gray-900">
                    {formatMoney(b.totalCost, b.currency)}
                  </td>
                  <td className="px-4 py-3">
                    <div className={`text-xs font-medium ${PAYMENT_COLOR[b.paymentStatus]}`}>
                      {b.paymentStatus}
                    </div>
                    {b.amountDue > 0 && (
                      <div className="text-xs text-gray-400">
                        Còn: {formatMoney(b.amountDue, b.currency)}
                      </div>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${STATUS_COLOR[b.status]}`}
                    >
